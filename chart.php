...
$result = mysqli_query($connect,"SELECT COUNT(*) AS `count` FROM `logs`");
$row = mysqli_fetch_assoc($result);
$count = $row['count'];

$debugT = mysqli_query($connect,"SELECT COUNT(*) AS `count` FROM `logs` WHERE level LIKE 'debug'");
$row = mysqli_fetch_assoc($debugT);
$debugT = $row['count'];

$infoT = mysqli_query($connect,"SELECT COUNT(*) AS `count` FROM `logs` WHERE level LIKE 'info'");
$row = mysqli_fetch_assoc($infoT);
$infoT = $row['count'];

$noticeT = mysqli_query($connect,"SELECT COUNT(*) AS `count` FROM `logs` WHERE level LIKE 'notice'");
$row = mysqli_fetch_assoc($noticeT);
$noticeT = $row['count'];

$warningT = mysqli_query($connect,"SELECT COUNT(*) AS `count` FROM `logs` WHERE level LIKE 'warning'");
$row = mysqli_fetch_assoc($warningT);
$warningT = $row['count'];

$erreurT = mysqli_query($connect,"SELECT COUNT(*) AS `count` FROM `logs` WHERE level LIKE 'erreur'");
$row = mysqli_fetch_assoc($erreurT);
$erreurT = $row['count'];

$criticalT = mysqli_query($connect,"SELECT COUNT(*) AS `count` FROM `logs` WHERE level LIKE 'critical'");
$row = mysqli_fetch_assoc($criticalT);
$criticalT = $row['count'];

$alertT = mysqli_query($connect,"SELECT COUNT(*) AS `count` FROM `logs` WHERE level LIKE 'alert'");
$row = mysqli_fetch_assoc($alertT);
$alertT = $row['count'];

$emergencyT = mysqli_query($connect,"SELECT COUNT(*) AS `count` FROM `logs` WHERE level LIKE 'emergency'");
...
<div class="alert alert-primary" role="alert" style="    background-color: #cce5ff;
  ...
  Les Logs de Tous les serveur du reseau .<br><br> Total :<?php echo $count ?> logs capturer
</div>
...
<div class="alert alert-primary" role="alert" style="    color: #856404;
    ...
    border-color: #ffeeba;">
Représentation graphique par le Diagramme en bâton
</div>
        </div>   
</div>
...
<div class="alert alert-primary" role="alert" style="    background-color: #cce5ff;
    border-color: #b8daff; color: #004085;"> 
  les serveurs dans Le reseau<br><br> Total :<?php echo $Totals++; ?> 
</div>
